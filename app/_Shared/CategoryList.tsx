export const CategoryList = [
    {
        id: 1,
        name: 'Programming',
        icon: '/CategoryLogo/programming.png',
        description: 'All about coding and software development'
    },
    {
        id: 2,
        name: 'Art',
        icon: '/CategoryLogo/art.png',
        description: 'Creative artworks and visual design'
    },
    {
        id: 3,
        name: 'Fitness',
        icon: '/CategoryLogo/lotus.png',
        description: 'Health, fitness, and physical well-being'
    },
    {
        id: 4,
        name: 'Music',
        icon: '/CategoryLogo/live-music.png',
        description: 'Music creation and performance'
    },
    {
        id: 5,
        name: 'Photography',
        icon: '/CategoryLogo/pictures.png',
        description: 'Capturing moments through photography'
    },
    {
        id: 6,
        name: 'Fashion',
        icon: '/CategoryLogo/brand.png',
        description: 'Latest trends in fashion and style'
    },
    {
        id: 7,
        name: 'Gaming',
        icon: '/CategoryLogo/game.png',
        description: 'Video games and interactive entertainment'
    },
    {
        id: 8,
        name: 'BBA',
        icon: '/CategoryLogo/bba.png',
        description: 'All about business and commerce'
    },

];
